...
initial begin
    $display("@%t, %m MSG: ****** test name: test_hs_dslp", $time);
    wait (`TB_INIT_DONE);
...
////`define SCU_RETCTRL_REG         `SCU_ADDR_BASE + 32'h0000_0028
    ...
    `CHIP_CPU_BFM.ahbrd(`SCU_PSOCTRL_REG, 3'd0, 4'd2, rdata,1,32'h0, 0);
    `CHIP_CPU_BFM.ahbwr(`HS_SCRATCH_BASE_REG+((hs_pu_st_idx+1)*4), 3'd0, 4'd2, 32'h0);

    `CHIP_CPU_BFM.ahbwr(`HS_SCRATCH_BASE_REG+((hs_pu_st_idx+2)*4), 3'd0, 4'd2, `SCU_CLKCFG1_REG);
    `CHIP_CPU_BFM.ahbwr(`HS_SCRATCH_BASE_REG+((hs_pu_st_idx+3)*4), 3'd0, 4'd2, 32'h0000_0101);


    // pwr dn, ldo, iso, ret
    `CHIP_CPU_BFM.ahbwr(`HS_SCRATCH_BASE_REG+(hs_pd_st_idx*4), 3'd0, 4'd2, `SCU_PSOCTRL_REG);
    `CHIP_CPU_BFM.ahbrd(`SCU_PSOCTRL_REG, 3'd0, 4'd2, rdata,1,32'h0,0);
    `CHIP_CPU_BFM.ahbwr(`HS_SCRATCH_BASE_REG+((hs_pd_st_idx+1)*4), 3'd0, 4'd2, 32'hffff_ffff);
    ...
    // ----------------------------
    // wait for HS_HIBERNATE
    wait_hs_st(HS_HIBERNATE);
...
task set_ext_reset;
begin
    $display("@%d, %m MSG: set_ext_reset", $time);
    force `CHIP_RSTN = 1'b0;
end
endtask
//// ----------------------------------------
task unset_ext_reset;
begin
    $display("@%d, %m MSG: unset_ext_reset", $time);
