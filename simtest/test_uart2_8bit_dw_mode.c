...
extern volatile uint32_t UART2_RXDM_INTR;
extern volatile uint32_t UART2_TXDM_INTR;
extern void set_port_uart2(void);
...
int test_uart2_8bit_dw_mode(void) {
    int ret = 0;
...
//      repeat(1000)@(posedge `CHIP_CLK48M); 
  ...
  join
    ...
    wr(UART2_ADDR_BASE+0x0, 0x00170101);
    ...
    wr(I2S_ADDR_BASE+0x1c, 0x00000100);
    //repeat(5)@(posedge `CHIP_CLK48M); 
    delayns(5);
    //`CHIP_CPU_BFM.ahbwr( `I2S_ADDR_BASE+32'h1c, 3'd0, 4'd2, 32'h0000_0000);
    wr(I2S_ADDR_BASE+0x1c, 0x00000000);
