...
#define APP_TIMER_SCHED_EVT_SIZE     sizeof(sw_timer_event_t)  /**< Size of button events being passed through the scheduler (is to be used for computing the maximum size of scheduler events). */
...
uint32_t app_timer_evt_schedule(sw_timer_timeout_handler_t timeout_handler,
                                void *                      p_context);
