...
//    return sd_flash_page_erase(p_op->erase.page);
	...
	TracerInfo("erase_execute_addr: 0x%x, page_num: 0x%x\r\n",dwEraseAdr, p_op->erase.page); 
...
static void on_operation_success(fs_op_t * const p_op)
{
    ...
    switch (p_op->op_code)
    {
        case FS_OP_STORE:
        {
            ...
            {
                drvi_EflashFlush(); //renew flash data
                  ...
                  void *                    p_context)
{
    ...
    p_op->erase.page           = (((uint32_t)p_page_addr - (uint32_t)p_config->p_start_addr) / FS_PAGE_SIZE);
