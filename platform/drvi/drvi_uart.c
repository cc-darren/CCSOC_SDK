...
static __inline int32_t DrviUringIsEmpty(T_UartRing *pUring)
{
    return (HEAD(pUring) == TAIL(pUring));
}

static __inline int32_t DrviUringGetCount(T_UartRing *pUring)
{
    return (HEAD(pUring) - TAIL(pUring));
}

int32_t DrviUringPop(T_UartRing *pUring, void *pData, int32_t iNum)
{
    int32_t iCnt1, iCnt2;
...
int32_t drvi_UartRx(uint8_t bPortNum, uint8_t bLen)
{
...
int32_t drvi_UartGet(T_UartPort *ptPort, uint8_t *pbBuf, int32_t iSize)
{
    int32_t iRingCount = 0;
    T_UartEvent *ptEvent;
...
int32_t drvi_Uart0Receive(uint8_t *pbBuf, int32_t iSize)
{
    int32_t iRingCount = 0;
...
int32_t drvi_Uart1Receive(uint8_t *pbBuf, int32_t iSize)
{
    int32_t iRingCount = 0;
...
int32_t drvi_Uart2Receive(uint8_t *pbBuf, int32_t iSize)
{
    int32_t iRingCount = 0;
