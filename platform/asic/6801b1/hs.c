...
//    }
    ...
    {
        //Switch clock to 2M
        //  If 48M is avail during PRD commands, 2M clock switch is not necessary
        ...
        HS_setHSCmd(Cmdset,AOCKGEN_SWRST_REG,tmpAOSWRST);
        HS_setHSCmd(Cmdset,AOCKGEN_CLKEN_REG,tmpAOClkEN);
      #endif
        
        tmpPSO |= 0x80600000;
        wr(HS_PSO_PDN_REG,(tmpPSO)); //0x80000000 this will be dones after all command. set bit31 before enter sleep and HS will clear this bit automatically
                                                //0x00600000 set CPU to power off in sleep period
    }
    else if (cmd_list_type == HS_CMDLIST_PUP3)
    {
        HS_setHSCmd(Cmdset,CKGEN_CLKEN_REG,tmpClkEN);
        HS_setHSCmd(Cmdset,AOCKGEN_CLKEN_REG,tmpAOClkEN);
    ...
    else
    {
        HS_setHSCmd(Cmdset,CKGEN_CLKEN_REG,tmpClkEN);
        HS_setHSCmd(Cmdset,AOCKGEN_CLKEN_REG,tmpAOClkEN);
        HS_setHSCmd(Cmdset,SCU_PSOCTRL_REG,(tmpPSO|0x00600000));
        HS_setHSCmd(Cmdset,SCU_ISOCTRL_REG,(tmpPSO|0x00600000));
        HS_setHSCmd(Cmdset,CKGEN_SWRST_REG,tmpSWRST);
        HS_setHSCmd(Cmdset,AOCKGEN_SWRST_REG,tmpAOSWRST);
    }
}
