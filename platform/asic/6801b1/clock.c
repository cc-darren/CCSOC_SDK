...
// Sec 8: declaration of static global variable
    ...
    const T_ClockConfig g_aClockTable[CLOCK_TOTAL_SUPPORTED] =
    {
        //16MHz
        {
            //XTAL=48MHz, 48/3=16MHz, we can select either PLL or XTAL
            (16000000),
          #if(USE_PLL == TRUE)
            (PLL_USE_MASK|PLL_POWER_UP_MASK|DIVCO_32MHZ_MASK|DIVM_32MHZ_MASK|DIVN_32MHZ_MASK),
            (SELECT_PLL_MASK|DCDC_GEN_CLK_DIV2_MASK|SYS_DIV_2_MASK),
          #else
        ...
        //24MHz
        {
            //XTAL=48MHz, 48/2=24MHz, we can select either PLL or XTAL
            (24000000),
          #if(USE_PLL == TRUE)
            (PLL_USE_MASK|PLL_POWER_UP_MASK|DIVCO_72MHZ_MASK|DIVM_72MHZ_MASK|DIVN_72MHZ_MASK),
            (SELECT_PLL_MASK|DCDC_GEN_CLK_DIV2_MASK|SYS_DIV_3_MASK),
          #else
            (PLL_BYPASS_MASK|PLL_POWER_DOWN_MASK|DIVCO_BYPASS_MASK|DIVM_BYPASS_MASK|DIVN_BYPASS_MASK),
        ...
        //48MHz
        {
            //XTAL=48MHz, 48/1=48MHz, we can select either PLL or XTAL
            (48000000),
          #if(USE_PLL == TRUE)
