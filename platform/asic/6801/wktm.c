...
{
    regWKTM0->dw.lowCounter = cnt;
}

void cc6801_timer1_counterLoad(uint32_t cnt)
{
    regWKTM1->dw.lowCounter = cnt;
}
...
void cc6801_timer0_init(T_callback handler)
{
    ...
    regWKTM0->dw.lowCounter = 0xFFFFFF;
...
void cc6801_timer1_init(T_callback handler)
{
    ...
    regWKTM1->dw.lowCounter = 0xFFFFFF;
...
void cc6801_timer0_start(void)
{
    ...
    regWKTM0->dw.lowCounter = 0xFFFFFF;    //default load 24bit counter to match Nordic design
    regWKTM0->bf.clear = 0;
    regWKTM0->bf.repeat = 1;                //start from repeat mode
