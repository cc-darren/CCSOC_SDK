...
#define MICROCHIP_24XX16_MAX_WRITE_SIZE 16
...
int microchip_24xx16_write(T_I2cDevice *ptDev,uint8_t bAddr, uint8_t *pbBuf, int iSize)
{
    int iWSize = 0;
    ...
    do
    {
        ...
        iRet = drvi_I2cWrite(ptDev, (uint8_t const *) &g_tI2c0TxBuffer, iWSize+sizeof(g_tI2c0TxBuffer.bAddr));
        if (CC_SUCCESS != iRet)
            continue;
...
void microchip_24xx16_read(T_I2cDevice *ptDev, uint8_t bAddr, uint8_t *pbBuf, int iSize)
{
    int iRet = CC_SUCCESS;

//    do
//    {
//        iRet = drvi_I2cWrite(ptDev, &bAddr, sizeof(bAddr));
//    } while(CC_SUCCESS != iRet);

//    do
//    {
//        iRet = drvi_I2cRead(ptDev, pbBuf, iSize);
//    } while(CC_SUCCESS != iRet);
    do
    {
        iRet = drvi_I2cWriteThenRead(ptDev, &bAddr, sizeof(bAddr), pbBuf, iSize);
    } while(CC_SUCCESS != iRet);
}


int TEST_I2cInit(void)
{
    return CC_SUCCESS;
}
...
//        memset(&g_tI2c0TxBuffer.baBuffer[60], 0x99, 4);
        ...
        microchip_24xx16_write(&i2c_config, bAddr, g_tI2c0TxBuffer.baBuffer, iSize);

        microchip_24xx16_read(&i2c_config, bAddr, g_baI2c0RxBuffer, iSize);

        if (memcmp(g_tI2c0TxBuffer.baBuffer, g_baI2c0RxBuffer, iSize))
            TracerInfo("I2C RW test Fail, w[0x%X],r[0x%X],s[%d]\r\n", g_tI2c0TxBuffer.baBuffer[0], g_baI2c0RxBuffer[0], iSize);
