...
void TEST_MemRW(uint32_t dwCount)
{
    ...
    while(dwIndex<dwCount)
    {
        ...
        TracerInfo("Retention RAM to System RAM...\n");
        dwSource = RET_RAM_START;    //from retention
        dwDest = SYS_RAM_START;      //to system memory
        iSize = SYS_RAM_SIZE;
...
//        test_mem(dwDest, dwSource, iSize);
        ...
        TracerInfo("System RAM to Retention RAM...\n");
        dwSource = SYS_RAM_START;    //from exchange memory
        dwDest = RET_RAM_START;      //to retention
        iSize = CHK_SIZE;
        test_mem(dwDest, dwSource, iSize);
