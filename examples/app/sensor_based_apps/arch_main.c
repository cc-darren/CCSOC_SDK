...
void CC_DB_Save_StartSleepService_Info(void)
{
    ...
    g_dwLast_SleepStage = 0;//CC_SLEEPMETER_AWAKE;
...
void _sensor_algorithm_sleepmeter_proc(void)
{
    ...
    {    
        ...
        S_AccInput _sInput;
        _sInput.wAcc[0] = _wAccelData[0];
        _sInput.wAcc[1] = _wAccelData[1];
        _sInput.wAcc[2] = _wAccelData[2];
        Alg_Sleep(_sInput);
...
static void _sensor_algorithm_liftarm_proc()
{
    ...
    if (eEnable== s_tVenusCB.cLiftarmEn)
    {
        if(1)
        {
            ...
            // Example liftarm result process....
            #if 0
            if (1 == _WinStatus)
            {
...
static void _sensor_algorithm_swimming_proc(void)
{
    ...
    S_AccInput _sInputData1; 
    S_GyroInput _sInputData2;
    S_MagInput _sInputData3;
    ...
    else
    {
        ...
        // User put mag data to buffer
        _sInputData3.wMag[0] = s_tMagRaw.AXIS_X;
        _sInputData3.wMag[1] = s_tMagRaw.AXIS_Y;
        _sInputData3.wMag[2] = s_tMagRaw.AXIS_Z;
        Alg_Swimming(_sInputData1,_sInputData2,_sInputData3);
    ...
    if(fifo_total_samples >= 260) // 260 samples(x,y,z) @5sec
    {
        ...
        for(int i = 0; i < fifo_total_samples; i++)
        {
            ...
            old_tick = Get_system_time_ms();
             ...
             // User put acc data to buffer
            _sInputData1.wAcc[0] = _wAccelData[0];
            _sInputData1.wAcc[1] = _wAccelData[1];
            _sInputData1.wAcc[2] = _wAccelData[2];
            
             // User put gryo data to buffer 
            _sInputData2.wGyro[0] = wgyro_data[0];
            _sInputData2.wGyro[1] = wgyro_data[1];
            _sInputData2.wGyro[2] = wgyro_data[2];
...
void CC_SYS_FactoryReset_Handler(void)
{
    ...
    s_tVenusCB.cLongsitEn = eEnable;
    s_tVenusCB.cLiftarmEn = eEnable;
    ...
    //Liftarm
#ifdef LIFTARM_EN
    s_tVenusCB.cLiftarmEn = eEnable; //CC_BLE_Cmd_GetLiftArmStatus();
    if ( eEnable==s_tVenusCB.cLiftarmEn )
    {
        //liftarm_open();
...
//#endif
    ...
    if (VENUS_EVENT_IS_ON(E_VENUS_EVENT_LONGSITEVENT) )
    {
        ...
        {   
            ...
            uint32_t UserInputData = 1;
            Alg_Longsit(UserInputData);
...
static void _AppAlgInit(void)
{
    ...
    //Default to eEnable Liftarm
    s_tVenusCB.cLiftarmEn =  eEnable; //CC_BLE_Cmd_GetLiftArmStatus();

    if (eEnable == s_tVenusCB.cLiftarmEn)
    {
        //liftarm_open();
        // User to Init Liftarm algo ...
        s_tVenusCB.stGeneralInfo.bBandLocation = CC_BLE_Cmd_GetLiftArmBandSetting();
        //liftarm_set_hangconfig(s_tVenusCB.stGeneralInfo.bBandLocation);
    }
    else
    {
        //liftarm_close();     // default disable liftarm
        // User to Disable Liftamr algo ...
    }
    
    #ifdef LONGSIT_EN
        //Default to eEnable Longsit
        s_tVenusCB.cLongsitEn = eEnable;
