...
#define NVDS_PARAMETER_MAX_LENGTH   NVDS_LEN_DEVICE_NAME
...
static void nvds_read(uint32_t address, uint32_t length, uint8_t *buf)
{
    ...
    memcpy(buf, (const void*)((uint32_t)nvds_env.nvds_space + address), length);
...
static void nvds_write(uint32_t address, uint32_t length, uint8_t *buf)
{
    ...
    drvi_EflashProgram((uint32_t)nvds_env.nvds_space + address, buf, length);
...
static void nvds_erase(uint32_t address, uint32_t length)
{
    ...
    drvi_EflashErasePage((uint32_t)nvds_env.nvds_space + address);
