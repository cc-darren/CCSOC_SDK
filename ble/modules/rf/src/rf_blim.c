...
void rf_init(struct rwip_rf_api *api)
{
  ...
  /* *************************************************************************************** */
  /*   TxON Access 1 -> PLL word (center freq)
  ...
  RF_ICTRX_EM_BLE_WR(txonptr,     txonptr+0xA);
  switch(icy_version)
  {
    case(0x31):   // CS2
    case(0x32):   // CS3
      RF_ICTRX_EM_BLE_WR(txonptr+0x2, 0x0C84);
      RF_ICTRX_EM_BLE_WR(txonptr+0x4, 0x0016);
  ...
  switch(icy_version)
    ...
    case(0x31):   // CS2
    case(0x32):   // CS3
      RF_ICTRX_EM_BLE_WR(txonptr+0xC, 0x0C81);
      RF_ICTRX_EM_BLE_WR(txonptr+0xE, 0x07C0);
    break;
    default:
    break;
    }
  ...
  /*   RxON Access 1 -> PLL word (centerfreq)
               -> Next Pointer = rxonptr+0x10
               -> Write Address @0x0C16 / 4 byte for CS2
               -> Write data = 0x00000000 -> Will be replaced by PLL word value from table in EM*/
  RF_ICTRX_EM_BLE_WR(rxonptr,  rxonptr+0xA);
  switch(icy_version)
    {
    case(0x31):   // CS2
    case(0x32):   // CS3
      RF_ICTRX_EM_BLE_WR(rxonptr+0x2, 0x0C84);
      RF_ICTRX_EM_BLE_WR(rxonptr+0x4,  0x0016);
      RF_ICTRX_EM_BLE_WR(rxonptr+0x6,  0x0000);
      RF_ICTRX_EM_BLE_WR(rxonptr+0x8,  0x0000);
    break;
    default:
    break;
    }

  /*   RxON Access 2 -> VCO SW Cap (v0x2-) or VCO SW Cap auto Calibration (v0x3-)
  ...
  RF_ICTRX_EM_BLE_WR(rxonptr+0xA,  rxonptr+0x10);
  switch(icy_version)
    {
    case(0x31):   // CS2
      RF_ICTRX_EM_BLE_WR(rxonptr+0xC,  0x0C81);
      RF_ICTRX_EM_BLE_WR(rxonptr+0xE,  0x00A2);
  break;
    case(0x32):   // CS3
      RF_ICTRX_EM_BLE_WR(rxonptr+0xC,  0x0C81);
      RF_ICTRX_EM_BLE_WR(rxonptr+0xE,  0x00A4);
    break;
    default:
    break;
    }

  /*   RxON Access 3 -> Rx enable (FSM Mode)
  ...
  switch(icy_version)
    ...
    case(0x32):   // CS3
      ...
      RF_ICTRX_EM_BLE_WR(rxonptr+0x14, 0x01C0);
  ...
  /*   RxON Access 4 -> Access Address Write (PATTERN) 
               -> Next Pointer = 0x0000 -> end of Sequence
  ...
  switch(icy_version)
    ...
    case(0x31):   // CS2
    case(0x32):   // CS2
      RF_ICTRX_EM_BLE_WR(rxonptr+0x1A, 0x0C84);
      RF_ICTRX_EM_BLE_WR(rxonptr+0x1C, 0x002C);
      RF_ICTRX_EM_BLE_WR(rxonptr+0x1E, 0x0000);
      RF_ICTRX_EM_BLE_WR(rxonptr+0x20, 0x0000);
    break;
    default:
    break;
    }
